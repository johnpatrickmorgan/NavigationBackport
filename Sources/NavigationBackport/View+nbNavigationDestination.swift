...
public extension View {
  ...
  func nbNavigationDestination<V>(isPresented: Binding<Bool>, @ViewBuilder destination: () -> V) -> some View where V: View {
    ...
    let binding = Binding(
      get: { isPresented.wrappedValue ? VoidHashable() : nil },
      set: { isActive, transaction in
        isPresented.transaction(transaction).wrappedValue = isActive != nil
      }
    )
    return modifier(
  ...
  ///     `destination` is currently presented.
  ///   - destination: A view to present.
  func nbNavigationDestination<Data: Hashable, V>(value: Binding<Data?>, @ViewBuilder destination: @escaping (Data) -> V) -> some View where V: View {
    return modifier(
      LocalDestinationBuilderModifier(
        data: value,
        builder: { AnyView(value.wrappedValue.map(destination)) }
      )
    )
  }
